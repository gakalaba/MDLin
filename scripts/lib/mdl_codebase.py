...
class MDLCodebase(ExperimentCodebase):
    ...
    def get_replica_cmd(self, config, i, run, local_exp_directory,
        ...
        replica_command += self.get_replication_protocol_arg_from_name(
        ...
        if config['server_cpuprofile']:
            replica_command += ' -cpuprofile %s' % os.path.join(
                exp_directory, config['out_directory_name'], 'server-%d-cpuprof-%d.prof' % (i, run))
        if 'server_blockprofile' in config and config['server_blockprofile']:
            replica_command += ' -blockprofile %s' % os.path.join(
                exp_directory, config['out_directory_name'], 'server-%d-blockprof-%d.prof' % (i, run))
        if 'server_memprofile' in config and config['server_memprofile']:
            replica_command += ' -memProfile %s' % os.path.join(
                exp_directory, config['out_directory_name'], 'server-%d-memprof-%d.prof' % (i, run))
