...
func main() {
  ...
  /////////////////////////////////////////
	...
	args := genericsmrproto.Propose{id, state.Command{state.PUT, 0, 0, 0}, 0} //Matt added a third field to Command
	...
	for j := 0; j < *rounds; j++ {
		...
		for i := 0; i < n+*eps; i++ {
			...
			if !*fast {
				...
				writers[leader].WriteByte(clientproto.GEN_PROPOSE) //OG has genericsmrproto.PROPOSE here
				args.Marshal(writers[leader])
			} else {
				//send to everyone
				for rep := 0; rep < N; rep++ {
					writers[rep].WriteByte(clientproto.GEN_PROPOSE) //OG has genericsmrproto.PROPOSE
