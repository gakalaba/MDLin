...
type Replica struct {
  ...
  totalEpochs	      int
  fanout		int
  condbuffermu  *sync.Mutex
  committedCondVar  *sync.Cond
}
...
func NewReplica(id int, peerAddrList []string, masterAddr string, masterPort int, thrifty bool,
	...
	r.committedCondVar = sync.NewCond(r.condbuffermu)
...
func (r *Replica) executeCommands() {
	...
	for !r.Shutdown {
		r.condbuffermu.Lock()
		if i > r.committedUpTo {
			r.committedCondVar.Wait()
		}
