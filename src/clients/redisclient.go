...
func (c *AsynchClient) AppRequest(opTypes []state.Operation, keys []int64, newValues []state.Value, oldValues []state.Value) (bool, state.Value) {
  ...
  } else if opTypes[0] == state.PUT || opTypes[0] == state.SET || opTypes[0] == state.INCR || opTypes[0] == state.SADD || opTypes[0] == state.HMGET || opTypes[0] == state.PUBLISH || opTypes[0] == state.SREM || opTypes[0]==state.SISMEMBER {
