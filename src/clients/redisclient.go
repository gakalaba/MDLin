...
func (c *AsynchClient) AppRequest(opTypes []state.Operation, keys []int64, newValues []state.Value, oldValues []state.Value) (bool, state.Value) {
  ...
  if opTypes[0] == state.GET || opTypes[0] == state.SCARD || opTypes[0] == state.SUBSCRIBE || opTypes[0] == state.LISTEN || opTypes[0] == state.EXISTS || opTypes[0] == state.HGETALL{
	c.Read(opTypes[0], key, state.NewString("0"))
  } else if opTypes[0] == state.PUT || opTypes[0] == state.SET || opTypes[0] == state.INCR || opTypes[0] == state.SADD || opTypes[0] == state.HMGET || opTypes[0] == state.PUBLISH || opTypes[0] == state.SREM || opTypes[0]==state.SISMEMBER || opTypes[0] == state.ZSCORE {
